...
class extra_batch:
    def __init__(self, edge_attr, focal_mask=None):
    ...
    def __len__(self):
        return self.focal_mask.size(0)


# collate_fn in Dataloader
# do padding to ensure the same input length
def vanilla_collator(items, max_node=128, max_dist=2, num_virtural_tokens=1, one_hot_edge=True):
    ...
    return batch, extra_batch(edge_attr, focal_mask)
